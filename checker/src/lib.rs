...
// ── Evaluation ──
// คะแนนบวก = ดำได้เปรียบ, ลบ = ขาวได้เปรียบ
fn evaluate(board: &Board) -> i32 {
    let mut score = 0i32;

    // 1. Material + Position value
    for sq in 1u8..=32 {
        let p = board.get(sq);
        ...
        let val: i32 = if p.is_king() {
            ...
            // โบนัสใกล้ฮอส
            let promo_bonus = if adv >= 6 { 100 }
                              else if adv >= 5 { 50 }
                              else if adv >= 4 { 20 } else { 0 };
            // โบนัสกลางกระดาน
            let center_bonus = if (c == 2 || c == 4) && (r >= 2 && r <= 5) { 15 }
                               else if (c == 1 || c == 3 || c == 5) && (r >= 2 && r <= 5) { 8 }
                               else { 0 };
            100 + adv * 12 + promo_bonus + center_bonus
        };
        if p.is_black() { score += val; } else { score -= val; }
    }

    // 2. ตรวจ "hanging pieces" — หมากที่ถูกกินฟรีโดยไม่มีการกินคืน
    //    นี่คือ key ที่ทำให้ AI ไม่แนะนำเดินให้กินฟรีโดยไม่ได้ประโยชน์
    let hanging_penalty = 80; // penalty ต่อ 1 ตัวที่ถูกกินฟรี

    // ตรวจหมากดำที่ขาวกินได้ฟรี
    for sq in 1u8..=32 {
        let p = board.get(sq);
        if !p.is_black() { continue; }
        // ตรวจว่าขาวกินหมากดำที่ sq ได้ไหม
        if can_capture_sq(board, sq, false) {
            // ถ้าดำกินคืนไม่ได้หลังจากขาวกิน
            if !can_recapture(board, sq, false) {
                score -= hanging_penalty;
            }
        }
    }

    // ตรวจหมากขาวที่ดำกินได้ฟรี
    for sq in 1u8..=32 {
        let p = board.get(sq);
        if !p.is_white() { continue; }
        if can_capture_sq(board, sq, true) {
            if !can_recapture(board, sq, true) {
                score += hanging_penalty;
            }
        }
    }
    ...
    // 4. Back row guard — ป้องกันฝั่งตัวเองไม่ให้ฝ่ายตรงข้ามเข้าฮอส
    for sq in 25u8..=32 {
        if board.get(sq).is_black() && !board.get(sq).is_king() { score += 25; }
    }
    for sq in 1u8..=8 {
        if board.get(sq).is_white() && !board.get(sq).is_king() { score -= 25; }
    }

    // 5. King safety — ฮอสที่อยู่มุมอาจถูกล็อก
    for sq in 1u8..=32 {
        let p = board.get(sq);
        if !p.is_king() { continue; }
        let km = gen_moves(board, p.is_black()).iter()
            .filter(|m| m.from == sq).count() as i32;
        if p.is_black() { score += km * 8; } else { score -= km * 8; }
    }

    score
}

// ตรวจว่าหมากที่ sq สามารถถูกฝ่าย attacker กินได้ไหม
fn can_capture_sq(board: &Board, sq: u8, attacker_is_black: bool) -> bool {
    let moves = gen_moves(board, attacker_is_black);
    moves.iter().any(|m| m.captures.contains(&sq))
}

// ตรวจว่าหลังจากฝ่าย attacker กินหมากที่ sq แล้ว ฝ่ายเจ้าของกินคืนได้ไหม
fn can_recapture(board: &Board, sq: u8, attacker_is_black: bool) -> bool {
    // หา move ที่ attacker กินหมากที่ sq
    let atk_moves = gen_moves(board, attacker_is_black);
    let cap_move = match atk_moves.iter().find(|m| m.captures.contains(&sq)) {
        Some(m) => m,
        None => return false,
    };
    // apply การกินนั้น
    let (new_board, _) = apply_move(board, cap_move, attacker_is_black);
    // ตรวจว่าฝ่ายเจ้าของ (defender) กินคืนได้ไหม
    let defender_is_black = !attacker_is_black;
    let def_moves = gen_moves(&new_board, defender_is_black);
    def_moves.iter().any(|m| !m.captures.is_empty())
}
